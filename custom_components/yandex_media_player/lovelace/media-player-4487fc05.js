...
class YmpBackground extends LitElement {
    constructor() {
         ...
         */
        this._cardHeight = 0;
        this._fallbackImage = '/y-media-player/no-image.png';
    }
...
:host .progress-bar .bar {
  height: 100%;
  width: 0;
  background-color: var(--ymp-accent-color);
...
var styles$6 = css`:host {
  ...
  --paper-slider-knob-color: var(--ymp-accent-color);
  --paper-slider-active-color: var(--ymp-accent-color);
  --paper-slider-pin-color: var(--ymp-accent-color);
...
var config$1 = {
	color: {
		label: "Accent color",
		helper: "The color of the volume setting and the progress bar.",
		options: {
			"default": "As in Home Assistant",
			orange: "Orange     🍊"
		}
	},
	devices: {
		label: "Choose Device",
		helper: "Devices that the media player will control."
	}
};
var media$1 = {
...
var config = {
	color: {
		...
		helper: "Цвет настройки громкости и прогресс бара.",
		options: {
			"default": "Как в Home Assistant",
			orange: "Апельсин     🍊"
		}
	},
	devices: {
		label: "Выберите Устройство",
		helper: "Устройства, которыми будет управлять медиаплеер."
	}
};
var media = {
...
class MediaPlayerCard extends LitElement {
    static async getConfigElement() {
        await import('./media-player-config-4bad9222.js');
...
window.customElements.define('y-media-player', MediaPlayerCard);
